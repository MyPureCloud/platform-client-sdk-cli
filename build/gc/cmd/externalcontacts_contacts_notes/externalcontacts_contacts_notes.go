...
func Cmdexternalcontacts_contacts_notes() *cobra.Command { 
  ...
  "content" : {
	...
	utils.AddFlag(listCmd.Flags(), "[]string", "expand", "", "which fields, if any, to expand. mergeset will include notes attached to ancestor contacts which have been merged into this one. Valid values: author, externalDataSources, division, mergeset")
  ...
  "content" : {
	...
	patchCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s\n%s", patchCmd.UsageTemplate(), "PATCH", "/api/v2/externalcontacts/contacts/{contactId}/notes/{noteId}", utils.FormatPermissions([]string{ "externalContacts:contact:edit",  }), utils.GenerateDevCentreLink("PATCH", "External Contacts", "/api/v2/externalcontacts/contacts/{contactId}/notes/{noteId}")))
  ...
  "content" : {
	...
	externalcontacts_contacts_notesCmd.AddCommand(patchCmd)
...
var patchCmd = &cobra.Command{
	Use:   "patch [contactId] [noteId]",
	Short: "Update a Contact Note",
	Long:  "Update a Contact Note",
	Args:  utils.DetermineArgs([]string{ "contactId", "noteId", }),
