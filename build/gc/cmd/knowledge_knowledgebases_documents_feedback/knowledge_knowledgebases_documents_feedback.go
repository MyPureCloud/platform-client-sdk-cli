...
func Cmdknowledge_knowledgebases_documents_feedback() *cobra.Command { 
  ...
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/KnowledgeDocumentFeedbackUpdateRequest"
      }
    }
  },
  "required" : false
  ...
  "content" : {
	...
	knowledge_knowledgebases_documents_feedbackCmd.AddCommand(updateCmd)
...
var updateCmd = &cobra.Command{
	Use:   "update [knowledgeBaseId] [documentId] [feedbackId]",
	Short: "Update feedback on a document",
	Long:  "Update feedback on a document",
	Args:  utils.DetermineArgs([]string{ "knowledgeBaseId", "documentId", "feedbackId", }),
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if printReqBody {
			...
			reqModel := models.Knowledgedocumentfeedbackupdaterequest{}
			utils.Render(reqModel.String())
