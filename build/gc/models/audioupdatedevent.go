package models
import (
    "time"
    "encoding/json"
    "strconv"
    "strings"
)

var (
    AudioupdatedeventMarshalled = false
)

// This struct is here to use the useless readonly properties so that their required imports don't throw an unused error (time, etc.)
type AudioupdatedeventDud struct { 
    


    


    


    


    

}

// Audioupdatedevent
type Audioupdatedevent struct { 
    // EventId - A unique (V4 UUID) eventId for this event
    EventId string `json:"eventId"`


    // EventDateTime - A Date Time representing the time this event occurred. Date time is represented as an ISO-8601 string. For example: yyyy-MM-ddTHH:mm:ss[.mmm]Z
    EventDateTime time.Time `json:"eventDateTime"`


    // ConversationId - A unique Id (V4 UUID) identifying this conversation
    ConversationId string `json:"conversationId"`


    // CommunicationId - A unique Id (V4 UUID) identifying this communication
    CommunicationId string `json:"communicationId"`


    // AudioState - The updated audioState for the target communication.
    AudioState Audiostate `json:"audioState"`

}

// String returns a JSON representation of the model
func (o *Audioupdatedevent) String() string {
    
    
    
    
    

    j, _ := json.Marshal(o)
    str, _ := strconv.Unquote(strings.Replace(strconv.Quote(string(j)), `\\u`, `\u`, -1))

    return str
}

func (u *Audioupdatedevent) MarshalJSON() ([]byte, error) {
    type Alias Audioupdatedevent

    if AudioupdatedeventMarshalled {
        return []byte("{}"), nil
    }
    AudioupdatedeventMarshalled = true

    return json.Marshal(&struct {
        
        EventId string `json:"eventId"`
        
        EventDateTime time.Time `json:"eventDateTime"`
        
        ConversationId string `json:"conversationId"`
        
        CommunicationId string `json:"communicationId"`
        
        AudioState Audiostate `json:"audioState"`
        *Alias
    }{

        


        


        


        


        

        Alias: (*Alias)(u),
    })
}

