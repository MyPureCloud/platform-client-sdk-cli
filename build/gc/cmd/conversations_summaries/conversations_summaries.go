...
func Cmdconversations_summaries() *cobra.Command { 
	getCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s\n%s", getCmd.UsageTemplate(), "GET", "/api/v2/conversations/{conversationId}/summaries", utils.FormatPermissions([]string{ "conversation:summary:view",  }), utils.GenerateDevCentreLink("GET", "Conversations", "/api/v2/conversations/{conversationId}/summaries")))
  ...
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/ConversationSummariesGetResponse"
      }
    }
  }
}`)
	conversations_summariesCmd.AddCommand(getCmd)
...
var getCmd = &cobra.Command{
	...
	Short: "Get the summaries of the conversation.",
	Long:  "Get the summaries of the conversation.",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		path := "/api/v2/conversations/{conversationId}/summaries"
