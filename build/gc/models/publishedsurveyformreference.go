package models
import (
    "encoding/json"
    "strconv"
    "strings"
)

var (
    PublishedsurveyformreferenceMarshalled = false
)

// This struct is here to use the useless readonly properties so that their required imports don't throw an unused error (time, etc.)
type PublishedsurveyformreferenceDud struct { 
    Id string `json:"id"`


    


    


    SelfUri string `json:"selfUri"`

}

// Publishedsurveyformreference
type Publishedsurveyformreference struct { 
    


    // Name
    Name string `json:"name"`


    // ContextId - The context id of this form.
    ContextId string `json:"contextId"`


    

}

// String returns a JSON representation of the model
func (o *Publishedsurveyformreference) String() string {
    
    
    
    
    
    
    
    
    
    
    
    
    

    j, _ := json.Marshal(o)
    str, _ := strconv.Unquote(strings.Replace(strconv.Quote(string(j)), `\\u`, `\u`, -1))

    return str
}

func (u *Publishedsurveyformreference) MarshalJSON() ([]byte, error) {
    type Alias Publishedsurveyformreference

    if PublishedsurveyformreferenceMarshalled {
        return []byte("{}"), nil
    }
    PublishedsurveyformreferenceMarshalled = true

    return json.Marshal(&struct { 
        
        
        Name string `json:"name"`
        
        ContextId string `json:"contextId"`
        
        
        
        *Alias
    }{
        

        

        

        

        

        

        

        

        
        Alias: (*Alias)(u),
    })
}

