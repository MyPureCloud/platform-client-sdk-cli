...
func Cmdtelephony_providers_edges_diagnostic_route() *cobra.Command { 
	createCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s", createCmd.UsageTemplate(), "POST", "/api/v2/telephony/providers/edges/{edgeId}/diagnostic/route", utils.FormatPermissions([]string{ "telephony:plugin:all",  })))
  ...
  &quot;schema&quot; : {
	...
	getCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s", getCmd.UsageTemplate(), "GET", "/api/v2/telephony/providers/edges/{edgeId}/diagnostic/route", utils.FormatPermissions([]string{ "telephony:plugin:all",  })))
