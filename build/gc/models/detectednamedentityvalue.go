package models
import (
    "encoding/json"
    "strconv"
    "strings"
)

var (
    DetectednamedentityvalueMarshalled = false
)

// This struct is here to use the useless readonly properties so that their required imports don't throw an unused error (time, etc.)
type DetectednamedentityvalueDud struct { 
    Raw string `json:"raw"`


    Resolved string `json:"resolved"`


    Unit string `json:"unit"`

}

// Detectednamedentityvalue
type Detectednamedentityvalue struct { 
    


    


    

}

// String returns a JSON representation of the model
func (o *Detectednamedentityvalue) String() string {

    j, _ := json.Marshal(o)
    str, _ := strconv.Unquote(strings.Replace(strconv.Quote(string(j)), `\\u`, `\u`, -1))

    return str
}

func (u *Detectednamedentityvalue) MarshalJSON() ([]byte, error) {
    type Alias Detectednamedentityvalue

    if DetectednamedentityvalueMarshalled {
        return []byte("{}"), nil
    }
    DetectednamedentityvalueMarshalled = true

    return json.Marshal(&struct {
        *Alias
    }{

        


        


        

        Alias: (*Alias)(u),
    })
}

