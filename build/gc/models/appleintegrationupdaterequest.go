...
var (
    AppleintegrationupdaterequestMarshalled = false
)

// This struct is here to use the useless readonly properties so that their required imports don't throw an unused error (time, etc.)
type AppleintegrationupdaterequestDud struct { 
    Id string `json:"id"`
...
func (u *Appleintegrationupdaterequest) MarshalJSON() ([]byte, error) {
    type Alias Appleintegrationupdaterequest

    if AppleintegrationupdaterequestMarshalled {
        return []byte("{}"), nil
    }
    AppleintegrationupdaterequestMarshalled = true
