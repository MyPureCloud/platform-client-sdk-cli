...
func Cmdrouting_sms_phonenumbers() *cobra.Command { 
  ...
  "content" : {
	...
	patchCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s\n%s", patchCmd.UsageTemplate(), "PATCH", "/api/v2/routing/sms/phonenumbers/{phoneNumberId}", utils.FormatPermissions([]string{ "sms:phoneNumber:edit",  }), utils.GenerateDevCentreLink("PATCH", "Routing", "/api/v2/routing/sms/phonenumbers/{phoneNumberId}")))
	utils.AddFileFlagIfUpsert(patchCmd.Flags(), "PATCH", `{
  "description" : "SmsPhoneNumberPatchRequest",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/SmsPhoneNumberPatchRequest"
      }
    }
  },
  "required" : true
  ...
  "content" : {
	...
	routing_sms_phonenumbersCmd.AddCommand(patchCmd)
...
var patchCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if printReqBody {
			...
			reqModel := models.Smsphonenumberpatchrequest{}
			utils.Render(reqModel.String())
