...
func Cmdworkforcemanagement_managementunits_timeoffrequests() *cobra.Command { 
	createCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s", createCmd.UsageTemplate(), "POST", "/api/v2/workforcemanagement/managementunits/{managementUnitId}/timeoffrequests", utils.FormatPermissions([]string{ "wfm:timeOffRequest:add",  })))
