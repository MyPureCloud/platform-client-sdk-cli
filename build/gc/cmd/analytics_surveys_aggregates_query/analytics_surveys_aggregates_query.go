...
func Cmdanalytics_surveys_aggregates_query() *cobra.Command { 
	createCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s\n%s", createCmd.UsageTemplate(), "POST", "/api/v2/analytics/surveys/aggregates/query", utils.FormatPermissions([]string{ "analytics:surveyAggregate:view",  }), utils.GenerateDevCentreLink("POST", "Analytics", "/api/v2/analytics/surveys/aggregates/query")))
	utils.AddFileFlagIfUpsert(createCmd.Flags(), "POST", `{
  "description" : "query",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/SurveyAggregationQuery"
      }
    }
  },
  "required" : true
  ...
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/SurveyAggregateQueryResponse"
      }
    }
  }
}`)
...
var createCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if printReqBody {
			...
			reqModel := models.Surveyaggregationquery{}
