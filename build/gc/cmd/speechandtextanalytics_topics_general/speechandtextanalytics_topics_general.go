...
func Cmdspeechandtextanalytics_topics_general() *cobra.Command { 
	utils.AddFlag(getCmd.Flags(), "string", "dialect", "", "The dialect of the general topics, dialect format is {language}-{country} where language follows ISO 639-1 standard and country follows ISO 3166-1 alpha 2 standard Valid values: en-US, es-US, en-AU, en-GB, en-ZA, es-ES, en-IN")
	getCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s", getCmd.UsageTemplate(), "GET", "/api/v2/speechandtextanalytics/topics/general", utils.FormatPermissions([]string{ "speechAndTextAnalytics:topic:view",  })))
