package employeeengagement_recognitions
...
func init() {
	CommandService = services.NewCommandService(employeeengagement_recognitionsCmd)
}

func Cmdemployeeengagement_recognitions() *cobra.Command { 
	createCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s\n%s", createCmd.UsageTemplate(), "POST", "/api/v2/employeeengagement/recognitions", utils.FormatPermissions([]string{ "engagement:recognition:add",  }), utils.GenerateDevCentreLink("POST", "Employee Engagement", "/api/v2/employeeengagement/recognitions")))
	utils.AddFileFlagIfUpsert(createCmd.Flags(), "POST", `{
  "description" : "Create Recognition",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/CreateRecognition"
      }
    }
  },
  "required" : true
  ...
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/RecognitionBase"
      }
    }
  }
}`)
	employeeengagement_recognitionsCmd.AddCommand(createCmd)

	getCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s\n%s", getCmd.UsageTemplate(), "GET", "/api/v2/employeeengagement/recognitions/{recognitionId}", utils.FormatPermissions([]string{ "engagement:recognition:view",  }), utils.GenerateDevCentreLink("GET", "Employee Engagement", "/api/v2/employeeengagement/recognitions/{recognitionId}")))
...
var createCmd = &cobra.Command{
	Use:   "create",
	Short: "Creates a recognition",
	Long:  "Creates a recognition",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if printReqBody {
			...
			reqModel := models.Createrecognition{}
...
var getCmd = &cobra.Command{
	Use:   "get [recognitionId]",
	Short: "Gets a single recognition",
	Long:  "Gets a single recognition",
	Args:  utils.DetermineArgs([]string{ "recognitionId", }),
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		path := "/api/v2/employeeengagement/recognitions/{recognitionId}"
		recognitionId, args := args[0], args[1:]
		path = strings.Replace(path, "{recognitionId}", fmt.Sprintf("%v", recognitionId), -1)
