...
var (
    TranscriptaggregationqueryMarshalled = false
)

// This struct is here to use the useless readonly properties so that their required imports don't throw an unused error (time, etc.)
type TranscriptaggregationqueryDud struct { 
...
func (u *Transcriptaggregationquery) MarshalJSON() ([]byte, error) {
    type Alias Transcriptaggregationquery

    if TranscriptaggregationqueryMarshalled {
        return []byte("{}"), nil
    }
