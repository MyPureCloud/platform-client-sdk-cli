...
func Cmdtaskmanagement_workitems_users_wrapups() *cobra.Command { 
	utils.AddFlag(getCmd.Flags(), "string", "expands", "", "Which fields, if any, to expand. Valid values: wrapupCode")
	utils.AddFlag(getCmd.Flags(), "string", "after", "", "The cursor that points to the end of the set of entities that has been returned.")
	utils.AddFlag(getCmd.Flags(), "int", "pageSize", "25", "Limit the number of entities to return. It is not guaranteed that the requested number of entities will be filled in a single request. If an `after` key is returned as part of the response it is possible that more entities that match the filter criteria exist. Maximum of 50.")
	utils.AddFlag(getCmd.Flags(), "string", "sortOrder", "descending", "Ascending or descending sort order Valid values: ascending, descending")
	getCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s\n%s", getCmd.UsageTemplate(), "GET", "/api/v2/taskmanagement/workitems/{workitemId}/users/{userId}/wrapups", utils.FormatPermissions([]string{ "workitems:wrapup:view",  }), utils.GenerateDevCentreLink("GET", "Task Management", "/api/v2/taskmanagement/workitems/{workitemId}/users/{userId}/wrapups")))
  ...
  "content" : {
	...
	taskmanagement_workitems_users_wrapupsCmd.AddCommand(getCmd)
