...
var (
	Description = utils.FormatUsageDescription("billing_contracts", "SWAGGER_OVERRIDE_/api/v2/billing/contracts", "SWAGGER_OVERRIDE_/api/v2/billing/contracts", )
	billing_contractsCmd = &cobra.Command{
		Use:   utils.FormatUsageDescription("billing_contracts"),
		Short: Description,
		Long:  Description,
	}
...
func init() {
	CommandService = services.NewCommandService(billing_contractsCmd)
}

func Cmdbilling_contracts() *cobra.Command { 
	getCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s\n%s", getCmd.UsageTemplate(), "GET", "/api/v2/billing/contracts/{contractId}", utils.FormatPermissions([]string{ "billing:subscription:view",  }), utils.GenerateDevCentreLink("GET", "Billing", "/api/v2/billing/contracts/{contractId}")))
  ...
  "content" : {
	...
	billing_contractsCmd.AddCommand(getCmd)
	...
	utils.AddFlag(listCmd.Flags(), "string", "externalNumber", "", "Filter by the unique external number.")
	listCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s\n%s", listCmd.UsageTemplate(), "GET", "/api/v2/billing/contracts", utils.FormatPermissions([]string{ "billing:subscription:view",  }), utils.GenerateDevCentreLink("GET", "Billing", "/api/v2/billing/contracts")))
  ...
  "content" : {
	...
	billing_contractsCmd.AddCommand(listCmd)
	return billing_contractsCmd
}
...
var getCmd = &cobra.Command{
	Use:   "get [contractId]",
	Short: "Get billing contract",
	Long:  "Get billing contract",
	Args:  utils.DetermineArgs([]string{ "contractId", }),
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		path := "/api/v2/billing/contracts/{contractId}"
...
var listCmd = &cobra.Command{
	Use:   "list",
	Short: "Get billing contracts",
	Long:  "Get billing contracts",
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		externalNumber := utils.GetFlag(cmd.Flags(), "string", "externalNumber")
		if externalNumber != "" {
