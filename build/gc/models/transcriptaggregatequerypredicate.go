...
var (
    TranscriptaggregatequerypredicateMarshalled = false
)

// This struct is here to use the useless readonly properties so that their required imports don't throw an unused error (time, etc.)
type TranscriptaggregatequerypredicateDud struct { 
...
func (u *Transcriptaggregatequerypredicate) MarshalJSON() ([]byte, error) {
    type Alias Transcriptaggregatequerypredicate

    if TranscriptaggregatequerypredicateMarshalled {
        return []byte("{}"), nil
    }
    TranscriptaggregatequerypredicateMarshalled = true
