...
var (
	Description = utils.FormatUsageDescription("outbound_dnclists_emailaddresses", "SWAGGER_OVERRIDE_/api/v2/outbound/dnclists/{dncListId}/emailaddresses", "SWAGGER_OVERRIDE_/api/v2/outbound/dnclists/{dncListId}/emailaddresses", "SWAGGER_OVERRIDE_/api/v2/outbound/dnclists/{dncListId}/emailaddresses", )
...
func Cmdoutbound_dnclists_emailaddresses() *cobra.Command { 
  ...
  "content" : { }
	...
	deleteCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s\n%s", deleteCmd.UsageTemplate(), "DELETE", "/api/v2/outbound/dnclists/{dncListId}/emailaddresses", utils.FormatPermissions([]string{ "outbound:dnc:delete",  }), utils.GenerateDevCentreLink("DELETE", "Outbound", "/api/v2/outbound/dnclists/{dncListId}/emailaddresses")))
	utils.AddFileFlagIfUpsert(deleteCmd.Flags(), "DELETE", ``)
	
	utils.AddPaginateFlagsIfListingResponse(deleteCmd.Flags(), "DELETE", `{
  "description" : "DNC email addresses deleted.",
  "content" : { }
}`)
	outbound_dnclists_emailaddressesCmd.AddCommand(deleteCmd)

	updateCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s\n%s", updateCmd.UsageTemplate(), "PATCH", "/api/v2/outbound/dnclists/{dncListId}/emailaddresses", utils.FormatPermissions([]string{ "outbound:dnc:edit",  }), utils.GenerateDevCentreLink("PATCH", "Outbound", "/api/v2/outbound/dnclists/{dncListId}/emailaddresses")))
	utils.AddFileFlagIfUpsert(updateCmd.Flags(), "PATCH", `{
  "description" : "DNC Emails",
  "content" : {
    "application/json" : {
      "schema" : {
        "$ref" : "#/components/schemas/DncPatchEmailsRequest"
      }
    }
  },
  "required" : true
  ...
  "content" : { }
	...
	outbound_dnclists_emailaddressesCmd.AddCommand(updateCmd)
...
var deleteCmd = &cobra.Command{
	...
	Short: "Deletes all or expired email addresses from a DNC list.",
	Long:  "Deletes all or expired email addresses from a DNC list.",
	Args:  utils.DetermineArgs([]string{ "dncListId", }),
...
var updateCmd = &cobra.Command{
	...
	Short: "Add emails to or Delete emails from a DNC list.",
	Long:  "Add emails to or Delete emails from a DNC list.",
	Args:  utils.DetermineArgs([]string{ "dncListId", }),
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if printReqBody {
			...
			reqModel := models.Dncpatchemailsrequest{}
			utils.Render(reqModel.String())
