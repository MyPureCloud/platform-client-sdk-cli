...
var (
    SurveyaggregationviewMarshalled = false
)

// This struct is here to use the useless readonly properties so that their required imports don't throw an unused error (time, etc.)
type SurveyaggregationviewDud struct { 
...
func (u *Surveyaggregationview) MarshalJSON() ([]byte, error) {
    type Alias Surveyaggregationview

    if SurveyaggregationviewMarshalled {
        return []byte("{}"), nil
    }
    SurveyaggregationviewMarshalled = true
