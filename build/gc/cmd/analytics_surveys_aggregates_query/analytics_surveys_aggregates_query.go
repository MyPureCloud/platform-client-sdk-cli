...
func Cmdanalytics_surveys_aggregates_query() *cobra.Command { 
	createCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s\n%s", createCmd.UsageTemplate(), "POST", "/api/v2/analytics/surveys/aggregates/query", utils.FormatPermissions([]string{ "analytics:surveyAggregate:view",  }), utils.GenerateDevCentreLink("POST", "Quality", "/api/v2/analytics/surveys/aggregates/query")))
