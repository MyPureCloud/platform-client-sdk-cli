...
func Cmdexternalcontacts_bulk_relationships_add() *cobra.Command { 
	createCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s", createCmd.UsageTemplate(), "POST", "/api/v2/externalcontacts/bulk/relationships/add", utils.FormatPermissions([]string{ "externalContacts:contact:add", "externalContacts:externalOrganization:add",  })))
