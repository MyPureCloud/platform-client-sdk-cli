...
var (
    CreatequeuerequestMarshalled = false
)

// This struct is here to use the useless readonly properties so that their required imports don't throw an unused error (time, etc.)
type CreatequeuerequestDud struct { 
    Id string `json:"id"`
...
func (u *Createqueuerequest) MarshalJSON() ([]byte, error) {
    type Alias Createqueuerequest

    if CreatequeuerequestMarshalled {
        return []byte("{}"), nil
    }
    CreatequeuerequestMarshalled = true
