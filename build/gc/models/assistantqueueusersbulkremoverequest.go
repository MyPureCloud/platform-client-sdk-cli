package models
import (
    "encoding/json"
    "strconv"
    "strings"
)

var (
    AssistantqueueusersbulkremoverequestMarshalled = false
)

// This struct is here to use the useless readonly properties so that their required imports don't throw an unused error (time, etc.)
type AssistantqueueusersbulkremoverequestDud struct { 
    

}

// Assistantqueueusersbulkremoverequest
type Assistantqueueusersbulkremoverequest struct { 
    // Entities - List of users to de-assign assistant form. Maximum users to remove is 100 per request.
    Entities []Assistantqueueuser `json:"entities"`

}

// String returns a JSON representation of the model
func (o *Assistantqueueusersbulkremoverequest) String() string {
     o.Entities = []Assistantqueueuser{{}} 

    j, _ := json.Marshal(o)
    str, _ := strconv.Unquote(strings.Replace(strconv.Quote(string(j)), `\\u`, `\u`, -1))

    return str
}

func (u *Assistantqueueusersbulkremoverequest) MarshalJSON() ([]byte, error) {
    type Alias Assistantqueueusersbulkremoverequest

    if AssistantqueueusersbulkremoverequestMarshalled {
        return []byte("{}"), nil
    }
    AssistantqueueusersbulkremoverequestMarshalled = true

    return json.Marshal(&struct {
        
        Entities []Assistantqueueuser `json:"entities"`
        *Alias
    }{

        
        Entities: []Assistantqueueuser{{}},
        

        Alias: (*Alias)(u),
    })
}

