...
func Cmdexternalcontacts_organizations_contacts() *cobra.Command { 
	...
	searchCmd.SetUsageTemplate(fmt.Sprintf("%s\nOperation:\n  %s %s\n%s", searchCmd.UsageTemplate(), "GET", "/api/v2/externalcontacts/organizations/{externalOrganizationId}/contacts", utils.FormatPermissions([]string{ "externalContacts:contact:view",  })))
