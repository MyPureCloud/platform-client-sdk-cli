...
var (
    WorkplanvalidationrequestMarshalled = false
)

// This struct is here to use the useless readonly properties so that their required imports don't throw an unused error (time, etc.)
type WorkplanvalidationrequestDud struct { 
    Id string `json:"id"`
...
func (u *Workplanvalidationrequest) MarshalJSON() ([]byte, error) {
    type Alias Workplanvalidationrequest

    if WorkplanvalidationrequestMarshalled {
        return []byte("{}"), nil
    }
